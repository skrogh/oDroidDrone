...
void GTEKF::updateCamera( const Matrix2Xd &points, const Matrix2Xd &prevPoints, cv::Mat debug ) {
	...
	if ( points.cols() >= 3 ) // only if 3 or more points (2 is needed, 1 extra for redundancy)
	{
		...
		//
		Vector4d h = estimateSimilarTransform( prevPointsOnGround, pointsOnGround );
		// only update, if estimated transform is finite
		if ( std::isfinite(h(0)) && std::isfinite(h(1)) && std::isfinite(h(2)) && std::isfinite(h(3)) ) {
			// remove scaling
			...
			Vector2d( h(2), h(3) ) - ( x.block<2,1>(4,0) - x.block<2,1>(4+ODO_STATE_SIZE,0) );
